...
/**
 * @author MurtazaAli
 * You need to create a virtual Command Prompt (VCP) for
 * managing virtual directories where a few operations/commands will be
 * supported.
 *
 */
public class CommandPrompt {
	...
	List<String> folders = new ArrayList<String>();// for keeping hierarchy of
													// folder
	...
	public void runVcp() {
		...
		while (true) {
			...
			printPath();// calling private method for printing current folder
						// path on command prompt
			inputCommand = input.nextLine();// taking input from cmd for command

			String[] commands = inputCommand.split(" ");// Splitting command
														// using spaces for
														// separating command
														// and folder name
	...
	/**
	 * @param folderName
	 * new folder name which is going to be create in this current
	 * folder
	 * 
	 * */
	public void mkdir(String folderName) {
		...
		// if folder is already exist with same name then new folder cann't
	...
	/**
	 * @param folderName,for finding folder recursively in current folder in
	 * it's sub folder as well
	 */
	...
	private String getPathOfFolder(Folder foundFolder) {
		...
		// setting currentFolder to foundFoder as from there only you will reach
		// to it's parent
		...
		// moving from current folder to towards it's parent and recursively
		// doing this until reach null because null is parent of root
		...
		// popping folders'name one by one as it will start from root because at
		// last root was pushed
	...
	public void ls() {
		...
		for (Folder subFolder : subFolders) {
			System.out.println(subFolder.getDateAndTime() + "  "
					+ subFolder.getName());
		}
	}
